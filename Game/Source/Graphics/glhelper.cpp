...
GLboolean GLHelper::keystateG = GL_FALSE;
...
bool GLHelper::init(GLint w, GLint h, std::string t) {
  ...
  glfwMakeContextCurrent(GLHelper::ptr_window); //OpenGL functions will affect whichever context is current
...
*/
void GLHelper::mousepos_cb(GLFWwindow*, double xpos , double ypos) {
#ifdef _DEBUG
...
*/
void GLHelper::error_cb(int , char const* description) {
#ifdef _DEBUG
...
*/
void GLHelper::fbsize_cb(GLFWwindow *, int w, int h) {
#ifdef _DEBUG
...
This function prints the necessary specifications
*/
void GLHelper::print_specs() {
    GLubyte const* str_vendor = glGetString(GL_VENDOR); //Get name of GPU vendor
    std::cout << "GPU Vendor: " << str_vendor << std::endl;

    GLubyte const* str_renderer = glGetString(GL_RENDERER); //Get name of OpenGL renderer
    std::cout << "GL Renderer: " << str_renderer << std::endl;

    GLubyte const* str_version_graphics = glGetString(GL_VERSION); //Get version number of OpenGL graphics driver
    std::cout << "GL Version: " << str_version_graphics << std::endl;

    GLubyte const* str_version_shading = glGetString(GL_SHADING_LANGUAGE_VERSION); //Get version number of OpenGL shading language
    std::cout << "GL Shader Version: " << str_version_shading << std::endl;

    GLint maj_num;
    glGetIntegerv(GL_MAJOR_VERSION, &maj_num); //Get major version number
    std::cout << "GL Major Version: " << maj_num << std::endl;

    GLint min_num;
    glGetIntegerv(GL_MINOR_VERSION, &min_num); //Get minor version number
    std::cout << "GL Minor Version: " << min_num << std::endl;

    GLint duo_buff;
    glGetIntegerv(GL_DOUBLEBUFFER, &duo_buff); //Check if double buffered
    if ((bool)duo_buff == true)
    {
    ...
    GLint max_vert;
    glGetIntegerv(GL_MAX_ELEMENTS_VERTICES, &max_vert); //Get maximum vertex count
    std::cout << "Maximum Vertex Count: " << max_vert << std::endl;
    
    GLint max_indices;
    glGetIntegerv(GL_MAX_ELEMENTS_INDICES, &max_indices); //Get maximum indices count
    std::cout << "Maximum Indices Count: " << max_indices << std::endl;
    
    GLint max_tex;
    glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_tex); //Get maximum texture size
    std::cout << "GL Maximum texture size: " << max_tex << std::endl;
    
    GLint max_view [4]; //GL_MAX_VIEWPORT_DIMS returns an array of 4 int values [x, y, width, height]
    glGetIntegerv(GL_MAX_VIEWPORT_DIMS, max_view); //Get maximum viewport dimensions
    std::cout << "Maximum Viewport Dimensions: " << max_view[0] << " x " << max_view[1] << std::endl;
}